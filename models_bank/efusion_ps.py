...
class EfusionPS(nn.Module):
    ...
    def forward(self, img, sparse_depth, mask, coors, k_nn_indices,  anns=None, sparse_depth_gt=None):
        ...
        if self.training:
            ...
            losses = {**losses, **proposal_losses,
                      **roi_losses, "depth_loss": loss}
            ...
            return None, [{**roi_result[idx], 'semantic_logits': semantic_logits[idx], 'depth': out[idx]} for idx, _ in enumerate(images)]
            # return None, out
        #
