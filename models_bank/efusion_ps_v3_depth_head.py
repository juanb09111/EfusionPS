...
class EfusionPS(nn.Module):
    def __init__(self, backbone_net_name,
        ...
        self.rgbd_conv = nn.Sequential(
            nn.Conv2d(in_channels=5, out_channels=32,
                      kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
    ...
    def forward(self,
        ...
        # rgbd branch
        x_concat_d_concat_f = torch.cat((images, sparse_depth, feat), dim=1) #C=5

        y_rgbdf = self.rgbd_conv(x_concat_d_concat_f)  # B x 32 x H/2 x W/2

        y_rgbdf_concat_y_sparse_concat_y_feat = torch.cat((y_rgbdf, y_sparse, y_feat), dim=1)

        y_rgbdf_concat_y_sparse_concat_y_feat = F.interpolate(y_rgbdf_concat_y_sparse_concat_y_feat, (H, W))

        

        fused, _, _, _ = self.fuse_conv(
            (y_rgbdf_concat_y_sparse_concat_y_feat, mask, coors, k_nn_indices))
...
# 
        if self.training:
            ...
            losses = {**losses, **maskrcnn_losses, "depth_loss": loss}
