...
def __update_model(trainer_engine, batch):
    ...
    imgs, ann, lidar_fov, masks, sparse_depth, k_nn_indices, sparse_depth_gt, _ = batch
...
if __name__ == "__main__":
    ...
    train_res_file = os.path.join(os.path.dirname(
        os.path.abspath(__file__)), constants.RES_LOC, constants.VKITTI_TRAIN_RES_FILENAME_EffPS)

    with open(train_res_file, "w+") as training_results:
        training_results.write(
            "----- TRAINING RESULTS - Vkitti EfficientPS----"+"\n")
    # Set device
    device = torch.device(
