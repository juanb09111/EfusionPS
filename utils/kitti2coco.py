...
def map_semseg_img(img, basename, semantic_map_dest):
    ...
    for i in range(h):
        for j in range(w):
            ...
            if (rgb != [0, 0, 0]).all():
                pix_cat = list(filter(lambda rgb2class_tup: (rgb2class_tup[1] == rgb).all(), rgb_2_class))[0]
                pix_cat = pix_cat[3]
    ...
    filename = "{}_{}_{}.png".format(semantic_map_dest, basename, timestamp)
    im.save(filename)

    return filename
    # r_im = Image.open("{}_{}_{}.png".format(root, basename, timestamp))
    # r_im_arr = np.asarray(r_im)
    
    # save_image(new_img_tensor, "{}_{}_{}.png".format(root, basename, timestamp))


def kitti_2_coco(rgb_root, instance_seg_root, semantic_seg_root, semantic_map_dest):

    data = {"info": {
        "year": 2021,
        "date_created": "2020-11-13T07:52:01Z",
        "version": "1.0",
        "description": "JD_NEW_DATA None",
        "contributor": "",
    ...
    for idx, img in enumerate(data["images"]):
        ...
        data["images"][idx]["semseg_img_filename"] = semseg_filename
        # print(semseg_img_arr.shape)
        ...
        for instance in unique_values:
            ...
            # break
        # break
    with open('kitti2coco_ann.json', 'w') as outfile:
...
#     __file__)), "../data_vkitti/virtual_world_vkitti-2/vkitti_2.0.3_instanceSegmentation/")


# semantic_root = os.path.join(os.path.dirname(os.path.abspath(
#     __file__)), "../data_vkitti/virtual_world_vkitti-2/vkitti_2.0.3_classSegmentation/")
